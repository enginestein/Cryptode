...
fn sha512(input: &[u8]) -> [u8; 64] {
    ...
    for (i, &word) in state.iter().enumerate() {
        result[i * 8] = ((word >> 56) & 0xFF) as u8;
        result[i * 8 + 1] = ((word >> 48) & 0xFF) as u8;
        result[i * 8 + 2] = ((word >> 40) & 0xFF) as u8;
        result[i * 8 + 3] = ((word >> 32) & 0xFF) as u8;
        result[i * 8 + 4] = ((word >> 24) & 0xFF) as u8;
        result[i * 8 + 5] = ((word >> 16) & 0xFF) as u8;
        result[i * 8 + 6] = ((word >> 8) & 0xFF) as u8;
        result[i * 8 + 7] = (word & 0xFF) as u8;
    }
...
fn pad_message(input: &[u8]) -> Vec<u8> {
    ...
    let padding_bytes = (112 - (message_len + 1) % BLOCK_SIZE) % BLOCK_SIZE;
    padded_message.extend(vec![0u8; padding_bytes]);

    let bit_length = (message_len as u128) << 3;
